...
public class ToolCallRecord {
    ...
    /**
     * 响应预览：这次调用回来的结果，已压成一行并截断。
     *
     * <p>列表卡片要"一眼看出这次回了个什么"，只看入参是不够的——同一份入参改过服务端之后
     * 可能这次成功、下次工具报错，而两次的入参完全一样。有了它，翻历史就等于翻一份
     * "什么时候变过"的时间线，不必逐条点进去看结果区。
     *
     * <p>只存<b>预览</b>而不是完整 result：完整 JSON 动辄几十 KB（一个读文件的内容块就够大了），
     * 而这里有几百条、每次调用都要写盘。选的是"人真正想先看一眼的那一段"
     * （结构化输出 / 首个文本内容块 / 错误信息），拼装逻辑在 {@code McpPanel}。
     *
     * <p>1.0.5 之前存下的老记录没有这个字段，读出来是空串——界面要按"没记录"处理，
     * 不要把它当成"响应是空的"。
     */
