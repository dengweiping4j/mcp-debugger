...
| **历史入参一键带入** | 每次调用都记下**当时真正发出去的入参**（成功、服务端报错、本地失败都记）；「历史」页签里点一下就把那一份填回 JSON 页签，直接重发或改一个值再试 |
...
```bash
./gradlew buildPlugin
# 产物： build/distributions/mcp-debug-idea-plugin-<版本>.zip（当前 1.0.4）
```
...
**「请求头」是参数区的一个页签，和参数 JSON、工具定义并列**（Postman 的 Params / Headers 也是这么排的）。
页签标题带条数（`请求头 (1)`），所以不点开也能一眼看出 token 到底在不在。它只对 `http` / `sse`
服务器出现，编辑内容就是一段 JSON：
...
改完**立刻**用于后续请求，不用重连（http 每次请求都重新读这份头；sse 的长连接是建连时带的头，
重连后才会换用新值）。写进去的就是服务端配置本身，所以导出配置、编辑服务器都不会把它丢掉——
「编辑服务器」对话框里因此不再重复提供入口。请求头挂在**服务器**上而不是某个工具上：
切来切去时它会停在当前页签不动，只有换服务器才重载内容。

**「历史」页签记下每次调用真正发出去的入参，点一下就能拿回来。**调试 MCP 工具时，
...
- **点条目就带入**（「带入参数」按钮效果相同）：内容写回「JSON」页签，**并自动切到该页签**——
  停在历史页上只会看到列表"一点没变"，容易以为点空了；切过去，改动就在眼前，
  「调用」也就在页签上方那条动作栏上，可以直接 `Ctrl+Enter` 重发。
  覆盖掉的内容可以 `Ctrl+Z` 撤销，动作栏上那行提示会写明这一点。
  代价是要连着试几条时得点回历史页——一处刻意换来的取舍。
- 每条只展示"时刻 + 结局"和一行 `键=值` 摘要；**完整入参与失败原因在悬停提示里**，
  「JSON」页签里更是原样可见。列表是索引，详情在别处——窄边栏里把整份 JSON 塞进列表
  反而什么都看不清。
- **成功、服务端报错（`isError`）、本地失败都记**：失败那一次的入参往往才是要复用的那一份。
  状态圆点分红 / 黄 / 绿三色，一眼能分出这次是"通了"还是"炸了"。
...
│   ├── ToolCallRecord.java     # 一次调用的历史记录（入参 + 结局），XmlSerializer 友好的纯字段 POJO
...
└── ui/
    ...
    ├── RequestHeadersPanel.java   # 「请求头」页签（仅 http / sse）：JSON 编辑区 → McpServerConfig.headers
    ├── CallHistoryPanel.java      # 「历史」页签：这个工具调过的入参，点条目即填回 JSON 页签
...
token 就在工具详情区「请求头」页签里改（`{"Authorization": "Bearer xxx"}`），改完立刻生效。

**Q：带鉴权的 http 服务端，第一次怎么连？**
A：先保存服务器（鉴权项没填、这次连接会失败是正常的），右侧会自动停在工具详情面板上——
点「请求头」页签填好 token，再点「连接」。没连上时请求头页签照样能编辑，就是为了这一步。

**Q：请求头为什么要填在页签里，而不是「编辑服务器」对话框里？**
A：token 是会跟着调试反复过期、反复换的东西，值得放在手边；配置对话框里的东西是"部署期"的，
每次改都要走"管理 → 编辑 → 保存 → 重连"。放进参数区页签后，它和参数 JSON 是同一级的两件事：
这次请求带什么参数、带什么头。

**Q：调过一遍的参数，下次还要重新填吗？**
A：不用。每次调用都会把**当时发出去的入参**记在「历史」页签里（成功、报错、失败都记），
点一下条目就填回 JSON 页签，改一个值再调即可。记录落在 IDE 配置目录的
`mcp-debugger-history.xml`，重启 IDE 也还在；每个工具保留最近 20 条。
