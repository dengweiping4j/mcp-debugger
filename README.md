...
拉取**全部工具**并列出；点开任意工具，按它自己的 JSON Schema 生成的表单填参数即可调用。
...
| **参数表单自动生成** | 工具 JSON Schema → 真实表单：枚举变下拉框、布尔变三态选择、嵌套对象分组折叠、数组按行填写；必填标记、默认值预填，`$ref` 与 `allOf` 会被展开 |
| **原始 JSON 调用** | 想直接写 JSON？切到 JSON 页签即可，以当前页签为准 |
| **结果好读** | 文本、结构化内容、内嵌资源、base64 图片都能直接看；原始 JSON-RPC 响应在隔壁页签，调用耗时显示在状态旁边 |
...
4. 左侧树里选中一个工具 → 右侧自动生成参数表单 → 填好点 **调用**。
5. 结果出现在下方卡片里；切到 **Raw** 页签能看完整 JSON-RPC 响应，**Console** 里是报文流水。
...
│  │   └ …                   │  │ 参数表单 │ JSON │ 结果             │  │
│  ├ 资源 (3)                │  │ ┌────────────────────────────────┐ │  │
│  ├ 资源模板 (1)            │  │ │ a: [________]  (number, 必填)   │ │  │
│  └ 提示词 (2)              │  │ │ b: [________]  (number, 必填)   │ │  │
│                            │  │ └────────────────────────────────┘ │  │
│                            │  │            [ 调用 ]                │  │
│                            │  ├────────────────────────────────────┤  │
│                            │  │ 结果：2 · 12 ms                    │  │
...
└── ui/
    ...
    ├── SchemaFormPanel.java       # JSON Schema → 表单
    ├── ResultView.java            # 结果渲染（文本/图片/资源）+ 导出
